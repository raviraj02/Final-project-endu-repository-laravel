          ...
          <tr class="text-gray-700 dark:text-gray-400">
            <td class="px-4 py-3">
              <div class="flex items-center text-sm">
                ...
                <div>
                  <p class="font-semibold">#00{{$order->id}}</p>
                </div>
              </div>
            </td>
            ...
            <td class="px-4 py-3 text-sm">
              @if ($order->status == 'pending')
              <form action="{{URL('update-status/'.$order->id)}}" method="post" enctype="multipart/form-data">
                @csrf
              ...
              @elseif ($order->status == 'complete')
              <span class="text-green-700"><i>Completed</i></span>  
              @elseif ($order->status == 'cancel')
              <span class="text-red-700"><i>Canceled</i></span>              
              @endif
              @if ($order->status == 'pending')
              <form action="{{URL('update-status/'.$order->id)}}" method="post" enctype="multipart/form-data">
                @csrf
